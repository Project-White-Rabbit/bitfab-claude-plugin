...
1. **If you already have a `sessionId` in context** from a previous `studio/open` step in this conversation, skip opening a new Studio. Instead, navigate the existing session to the desired page:
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/node_modules/bitfab-plugin-lib/dist/commands/navigateStudio.js" <sessionId> <initialPath>
   ```
   ...
   **The `initialPath` MUST start with `/studio`.** Never pass `/`, a bare URL, or any path outside the `/studio/` route tree. Omit the argument entirely to default to `/studio`.
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/dist/commands/openStudio.js" <initialPath> [agentSessionId]
   ```
   ...
   **Recovering after compaction:** If the Studio is already open from a prior context window but you've lost the `sessionId`, recover it:
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/node_modules/bitfab-plugin-lib/dist/commands/recoverStudio.js" <agentSessionId>
   ```
...
2. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Reviewing dataset"`.
...
6. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Searching traces"`.
...
8. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Labeling traces"`.
...
4. **Upgrade the SDK and replay script.** The replay script references SDK APIs (`experimentGroupId`, `codeChangeDescription`, per-item `traceId`) that require a recent SDK. Upgrade the SDK first, then regenerate the script.
   ...
   **2. Regenerate the replay script.** Invoke `/bitfab:setup replay` using the Skill tool to regenerate the script with full flag support.
...
5. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Making changes"`.
...
6. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Running replay"`.
   ...
   **Open the experiments page before running replay (if experiment groups are supported).** If `supportsExperimentGroups` is true AND the Studio is still open (the background process hasn't emitted `session-ended`), navigate it to the experiments page using the group ID:
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}/node_modules/bitfab-plugin-lib/dist/commands/navigateStudio.js" <sessionId> "/studio/experiments?experimentGroupId=<experimentGroupId>"
   ```
...
9. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Evaluating results"`.
...
10. **Studio activity:** If `studioMode` is true, run `node "${CLAUDE_PLUGIN_ROOT}/dist/commands/pushActivity.js" <sessionId> started "Evaluating results"`.
