---
description: Iterate on Bitfab span-rendering templates for a specific trace function, with the agent editing via MCP and the user previewing in the browser
...
Each round of the loop. **Every `mcp__plugin_bitfab_Bitfab__get_template` and `mcp__plugin_bitfab_Bitfab__update_template` call must include `traceFunctionKey: <key>`** (the key picked in step 1); without it you'd edit the org-global instead of this function's override.
...
4. Call `mcp__plugin_bitfab_Bitfab__update_template` with `spanType`, `traceFunctionKey: <key>`, and the full edited body. The tool upserts the per-function row in place (no version bump, no row juggling). On the first save for a span type the row is created; subsequent edits update it.
