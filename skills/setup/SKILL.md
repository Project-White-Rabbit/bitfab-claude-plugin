...
argument-hint: "[wizard|explain|login|session-logs|instrument|modify|inspect|switch-org|replay|cloud|db-snapshot|templates|analyze-repo] [what to do]"
---
...
- Mode `cloud`: invoke the `setup-cloud` skill with mode `cloud`, forwarding `$ARGUMENTS` minus the leading mode keyword (if the user typed one).
