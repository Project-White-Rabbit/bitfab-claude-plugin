...
3. **Compare coverage.** Replay is non-interactive once entered, do not ask the user whether to create or add scripts. Determine which case applies:
   ...
   **Next:**
   ...
   - Every trace function key already has a conformant replay script (coverage complete) (mode `wizard` or `replay`): invoke the `setup-cleanup` skill with the current mode (`wizard` or `replay`).
