...
}, async ({ traceFunctionKey, status, type, verbose }) => proxyToolCall("list_graders", { traceFunctionKey, status, type, verbose }));
server.tool("search_traces", "Search and filter traces for a traced function. Returns matching traces with IDs, status, timestamps, and output preview. Supports keyword search, date range, status filter, regex matching, and grader evaluation filters. Use drillDown to refine previous results.", {
...
server.tool("read_traces", 'Read one or more traces by ID — includes inputs, outputs, and status. By default returns span summaries; set scope="full" to get full span details (input, output, reasoning, context, errors).', {
    traceIds: z
        .array(z.uuid())
        .min(1)
        .max(10)
