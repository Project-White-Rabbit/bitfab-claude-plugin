...
| `/bitfab:assistant benchmark <key> [<dataset-id>] [studio]` | Phase 5 (pick-dataset step) | Phase Benchmark (terminal scorecard; no edits to the function). Optional `studio` keyword opens the experiments page and streams verdicts live |
...
10. **Studio lifecycle and universal cleanup.** Phase 5 opens Studio at the start (`openStudioTo.js`, background) and kills the background process at the end (in `benchmark` mode this happens only when the run opted in with the `studio` keyword; without it, no Studio is opened and the cleanup step is a no-op). Additionally, every terminal exit in the flow routes through the `cleanup/close-studio` step, which closes the Studio browser tab via `closeStudio.js <sessionId>` if a session was opened, or is a no-op if not. This is enforced structurally by the flow (every `next: null` points to `cleanup/close-studio`), not by a behavioral instruction. The experiment viewer in Step 4 uses `openStudioTo.js` to navigate the existing Studio or open a new one. The agent's textual summary in Step 5 is still required and is not optional.
